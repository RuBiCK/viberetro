...
interface SessionContextType {
  ...
  toggleVote: (targetId: string, targetType: 'card' | 'cluster') => void;
  ...
  getTypingUsers: (cardId: string) => User[];
}
...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    newSocket.on('stage:changed', (data: { stage: SessionStage }) => {
      console.log('🎭 Stage changed received:', data.stage);
      setSession(prev => {
        console.log('🎭 Updating session stage from', prev?.stage, 'to', data.stage);
        return prev ? { ...prev, stage: data.stage } : null;
      });
    });
    ...
    newSocket.on('typing:broadcast', (data: { userId: string; cardId: string; isTyping: boolean }) => {
      if (data.isTyping) {
        setTypingIndicators(prev => {
          // Remove existing indicator for this user/card combo
          const filtered = prev.filter(t => !(t.userId === data.userId && t.cardId === data.cardId));
          // Add new indicator
          return [...filtered, { userId: data.userId, cardId: data.cardId, timestamp: Date.now() }];
        });
      } else {
        setTypingIndicators(prev => prev.filter(t => !(t.userId === data.userId && t.cardId === data.cardId)));
      }
    });
    ...
    const typingCleanupInterval = setInterval(() => {
      const now = Date.now();
      setTypingIndicators(prev => prev.filter(t => now - t.timestamp < 3000));
    }, 1000);
    ...
    return () => {
      ...
      newSocket.off('typing:broadcast');
  ...
  const hasUserVoted = useCallback((targetId: string): boolean => {
    if (!currentUser) return false;
    return votes.some(v => v.userId === currentUser.id && v.targetId === targetId);
  }, [votes, currentUser]);

  const toggleVote = useCallback((targetId: string, targetType: 'card' | 'cluster') => {
    if (!currentUser) return;

    // Check if user has already voted for this target
    const existingVote = votes.find(v => v.userId === currentUser.id && v.targetId === targetId);

    if (existingVote) {
      // Remove the vote
      socket?.emit('vote:remove', { voteId: existingVote.id });
    } else {
      // Cast new vote
      socket?.emit('vote:cast', { targetId, targetType });
    }
  }, [socket, votes, currentUser]);

  const createActionItem = useCallback((owner: string, task: string) => {
    socket?.emit('action:create', { owner, task, completed: false });
  }, [socket]);
  ...
  const startTyping = useCallback((cardId: string) => {
  ...
  const getTypingUsers = useCallback((cardId: string): User[] => {
    const typingUserIds = typingIndicators
      .filter(t => t.cardId === cardId)
      .map(t => t.userId);
    return users.filter(u => typingUserIds.includes(u.id));
  }, [typingIndicators, users]);
